...
if __name__ == "__main__":
    ...
    for ax, (case_name, res) in zip(axes, selected_cases):
        ...
        ax.scatter(X_trunc[0, 0], X_trunc[0, 1], color='black', marker='o', s=40, label='Start Point', zorder=10)
        
        if error_index is not None:
            ax.scatter(trajectory[-1, 0], trajectory[-1, 1], color='red', label=f'Error > {error_th}x range (step {error_index})', zorder=5)
            
        ax.set_title(f"{case_name} (Traj {traj_idx})\nRMSE: {rmse:.2e}")
        ax.set_xlabel("x")
        ax.set_ylabel("y")
    ...
    fig.suptitle(f"Trajectory Rollouts\n{model_label} | Avg Test RMSE: {avg_rmse:.2e}", fontsize=14, y=0.98)
